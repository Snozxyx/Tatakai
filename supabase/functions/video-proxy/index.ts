...
serve(async (req) => {
  ...
  try {
    ...
    if (type === 'video') {
      ...
      const isHTML = upstreamContentType.includes('text/html') || upstreamContentType.includes('application/xhtml');
      ...
      if (isHTML) {
        ...
        // Strip CSP meta tags to prevent "default-src 'none'" blocking inline styles/scripts
        text = text.replace(/<meta\s+http-equiv=["']Content-Security-Policy["'][^>]*>/gi, '<!-- CSP removed -->');
        text = text.replace(/<meta\s+content=["'][^"']*["']\s+http-equiv=["']Content-Security-Policy["'][^>]*>/gi, '<!-- CSP removed -->');

        // Inject protection script to neutralize click-jacking and popups
        const protectionScript = `
        <script>
          /* Tatakai Ad-Shield */
          (function() {
            console.log("[Tatakai] Protection active");
            
            // Override window.open to block popups
            window.open = function() { 
              console.log("[Tatakai] Blocked window.open attempt"); 
              return { closed: true, focus: function(){} }; 
            };
            
            // Prevent event listener hijacking (document.onclick = ui)
            Object.defineProperty(document, 'onclick', {
              set: function(val) { console.log("[Tatakai] Blocked document.onclick assignment"); },
              get: function() { return null; },
              configurable: true
            });
            Object.defineProperty(document, 'ontouchend', {
              set: function(val) { console.log("[Tatakai] Blocked document.ontouchend assignment"); },
              get: function() { return null; },
              configurable: true
            });
            
            // Neutalize AdBlock detection
        ...
        // Inject script at the top of head
        if (text.includes('<head>')) {
          text = text.replace('<head>', '<head>' + protectionScript);
        } else if (text.includes('<body>')) {
          text = text.replace('<body>', '<body>' + protectionScript);
        } else {
          text = protectionScript + text;
        }

        // Sanitize source code: neutralize specific known malicious patterns
        text = text.replace(/document\.onclick\s*=\s*ui/g, '// document.onclick = ui // blocked by Tatakai');
        text = text.replace(/document\.ontouchend\s*=\s*ui/g, '// document.ontouchend = ui // blocked by Tatakai');
