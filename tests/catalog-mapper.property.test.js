 ...
 *
 * Rationale: An exact hint always returns `confidence: 100`. Without a hint
 * the mapper runs a fuzzy/synonym search that returns 0–100. Therefore the
 * with-hint confidence (100) is always ≥ the without-hint confidence (0–100).
 *
...
function makeAniListResponse(id, romaji, confidence = 'low') {
  // Use a title that will produce a known confidence level:
  //   'low'  → title that won't match the query well (score < 80)
  //   'high' → title that exactly matches the query (score = 100)
  const title = confidence === 'high' ? romaji : `ZZZ_NoMatch_${id}`;
  return {
    data: {
      Page: {
        media: [
...
/** Build a minimal ParsedRelease from a title string. */
 ...
 * We constrain to printable ASCII to keep titles realistic and avoid
...
describe('mapParsedReleaseToAnime() — Property 11: confidence monotonicity (unit examples)', () => {
  it('confidence with hint (100) >= confidence without hint when AniList returns no match (0)', async () => {
    ...
    const parsed = makeParsed('Attack on Titan MonotonicityTest1');
  ...
  it('confidence with hint (100) >= confidence without hint when AniList returns partial match', async () => {
    ...
    mockFetch(makeAniListResponse(16498, 'Shingeki no Kyojin', 'low'));

    const parsed = makeParsed('Attack on Titan MonotonicityTest2');
    const withoutHint = await mapParsedReleaseToAnime(parsed);
  ...
  it('confidence with hint (100) >= confidence without hint when AniList returns high-confidence match', async () => {
    ...
    mockFetch(makeAniListResponse(99999, title, 'high'));

    const parsed = makeParsed(title);
    const withoutHint = await mapParsedReleaseToAnime(parsed);
  ...
  it('confidence with hint (100) >= confidence without hint when network fails', async () => {
    ...
    const parsed = makeParsed('NetworkFailTitle MonotonicityTest4');
    const withoutHint = await mapParsedReleaseToAnime(parsed);
  ...
  it('with-hint result always has method "exact"', async () => {
    ...
    const parsed = makeParsed('MethodCheckTitle');
  ...
  it('without-hint confidence is in [0, 100]', async () => {
    ...
    const parsed = makeParsed('RangeCheckTitle MonotonicityTest5');
  ...
  it('confidence(with hint) >= confidence(without hint) for any ParsedRelease with non-null title', async () => {
    ...
    await fc.assert(
      fc.asyncProperty(parsedReleaseArb, anilistIdArb, async (parsed, anilistId) => {
        ...
        // We append the anilistId to the title to ensure each run uses a
  ...
  it('confidence(with hint) >= confidence(without hint) when AniList returns a partial match', async () => {
    await fc.assert(
      fc.asyncProperty(parsedReleaseArb, anilistIdArb, async (parsed, anilistId) => {
        ...
        mockFetch(makeAniListResponse(anilistId, `ZZZ_NoMatch_${anilistId}`, 'low'));
        const withoutHint = await mapParsedReleaseToAnime(uniqueParsed);
