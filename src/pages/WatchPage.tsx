...
export default function WatchPage() {
  ...
  const selectedSource = useMemo(() => {
    ...
    return null;
  }, [sourcesData?.sources, selectedServerIndex, selectedLangCode, availableServers]);
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        {availableServers.map((server, idx) => {
                          // Skip duplicates or TatakaiAPI if valid servers exist
                        ...
                        {/* M3U8 Custom Sources (Animelok, Animeya HLS) */}
                        {sourcesData?.sources
                          .filter(s => s.isM3U8 && !s.isEmbed && s.providerName && s.providerName !== 'TatakaiAPI')
                          .map((source, idx) => (
                            <button
                              key={`${source.langCode}-${idx}`}
                              onClick={() => {
                                // Find exactly this source
                                const sourceIdx = sourcesData.sources.indexOf(source);
                                if (sourceIdx !== -1) {
                              ...
                              className={`h-9 px-3 md:px-4 rounded-xl font-medium transition-all text-sm relative overflow-hidden ${selectedServerIndex === -3 && selectedLangCode === source.langCode
                                ? "bg-foreground text-background shadow-lg"
                                : source.providerName?.includes('Multi')
                      ...
                      {/* Embed Sources (Separate section) */}
                      {sourcesData?.sources.some(s => s.isEmbed && s.providerName) && (
                        <div className="flex flex-col gap-2">
                          <div className="text-xs text-muted-foreground font-medium">Embed Sources</div>
                          <div className="flex flex-wrap gap-2">
                            {sourcesData.sources
                              .filter(s => s.isEmbed && s.providerName)
                              .reduce((acc: Array<{ name: string, code: string, provider: string }>, source) => {
                                if (!acc.find(l => l.code === source.langCode)) {
                                  const serverInfo = getAnimeServerName(source.langCode || '', source.providerName);
                                  acc.push({
                                    name: serverInfo.name,
                                    code: source.langCode!,
                                    provider: serverInfo.description
                                  });
                                }
                                return acc;
                              }, [])
                              .map((item) => (
                                <button
                                  key={item.code}
                                  onClick={() => {
                                    const sourceIdx = sourcesData.sources.findIndex(s => s.langCode === item.code);
                                    if (sourceIdx !== -1) {
                                      setSelectedServerIndex(-2); // Embed index
                                      setSelectedLangCode(item.code);
                                      setFailedServers(new Set());
                                    }
                                  }}
                                  title={item.provider}
                                  className={`h-9 px-3 md:px-4 rounded-xl font-medium transition-all text-sm flex items-center gap-1 ${selectedServerIndex === -2 && selectedLangCode === item.code
                                    ? "bg-foreground text-background shadow-lg"
