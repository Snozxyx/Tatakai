...
export default function WatchRoomPage() {
    ...
    const handleTransferHost = async (targetUserId: string, displayName: string) => {
        ...
        if (!confirm(`Transfer host controls to ${displayName}?`)) return;
    ...
    const handleCreatePoll = async () => {
        ...
        if (!normalizedQuestion) {
            toast.error('Add a poll question first');
            return;
        }
        ...
        await sendMessage.mutateAsync({
            roomId,
            message: `New poll: ${normalizedQuestion}`,
            messageType: 'system',
        });
                ...
                {(isParticipant || isHost) && (
                    <div className="mb-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <GlassPanel className="p-4">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-[10px] font-black uppercase text-muted-foreground flex items-center gap-2">
                                    <ListOrdered className="w-3 h-3 text-primary" /> Watch Queue ({queueItems.length})
                                </h3>
                                {isHost && (
                            ...
                            {queueItems.length === 0 ? (
                                <p className="text-xs text-muted-foreground">Queue is empty. Add episodes for a smoother group session.</p>
                            ) : (
                                        ...
                                        <div key={item.id} className="flex items-center gap-2 p-2 rounded-lg bg-black/20 border border-white/5">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <p className="text-[10px] text-muted-foreground truncate">
                                                    {item.episode_number ? `Episode ${item.episode_number}` : 'Next available episode'}
                                                </p>
                                            </div>
                                            {isHost && (
                        ...
                        <GlassPanel className="p-4">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="text-[10px] font-black uppercase text-muted-foreground flex items-center gap-2">
                                    <BarChart3 className="w-3 h-3 text-primary" /> Group Poll
                                </h3>
                                {isHost && activePoll && (
                                    <Button
                                        size="sm"
                            ...
                            {activePoll ? (
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center justify-between text-xs font-semibold">
                                                    ...
                                                    <span>{result.votes} votes ({result.percent}%)</span>
                                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <Button size="sm" onClick={handleCreatePoll} disabled={createRoomPoll.isPending}>
                                        Create 5-Minute Poll
                                    </Button>
                                </div>
                            ) : (
                                <p className="text-xs text-muted-foreground">No active poll yet. Host can start one anytime.</p>
                            )}
                        </GlassPanel>
                    </div>
                )}
