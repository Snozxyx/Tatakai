...
async function fetchMangaSection(config: typeof SECTION_CONFIGS[0], _pageSeed: number, canShowAdult: boolean): Promise<MangaSection> {
  const query = config.query || `${config.genre} manga`;
  
  try {
    const data = await searchManga(query, 1, 20);
    const rawItems = Array.isArray(data?.results) ? data.results : [];
    const items = dedupeItems(mapSearchRowsToItems(rawItems, canShowAdult)).slice(0, FIXED_SECTION_ITEMS);
