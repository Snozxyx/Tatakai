...
import { useWatchlist } from '../user/useWatchlist';
import { useWatchHistory } from '../user/useWatchHistory';
import { AnimeCard } from '@/lib/api';
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RecommendationResult[]> => {
      ...
      for (const genre of topGenres) {
        try {
          const results = await contentGraph.search({ query: genre, page: 1, perPage: 20 });
          if (results?.media) {
            for (const anime of results.media.map(toAnimeCard)) {
...
export function useQuickRecommendations(limit: number = 10) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<AnimeCard[]> => {
      ...
      if (topGenre) {
        try {
          const results = await contentGraph.search({ query: topGenre, page: 1, perPage: limit * 2 });
          const watchedIds = new Set(watchlist.map(w => w.anime_id));

          return (results?.media || []).map(toAnimeCard)
