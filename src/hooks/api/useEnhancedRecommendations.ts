...
import { useWatchHistory } from '../user/useWatchHistory';  //src\hooks\user\useWatchHistory.ts
import { AnimeCard } from '@/lib/api';
import { contentGraph, toHomeData } from '@/core';
...
async function findSimilarAnime(
  ...
  const results = await contentGraph.search({ query: animeId, page: 1, perPage: limit + 1 });
  return (results.media || [])
    .map((m) => ({
      ...
      malId: m.malId || undefined,
      anilistId: m.anilistId || undefined,
    }))
